...
The only thing missing is a convention about *where things go and who writes them*. That includes not just specs and tasks, but also local decisions, conventions, learnings, and operational guides. That's the standard's entire job. Once the convention is in place, you have both an agent-readable source of truth and a human-agent workspace with zero additional infrastructure, zero auth layer, zero latency, zero operational cost.
...
Latent work belongs in skills layered above the kernel: `review-product`, `clarify-idea`, `write-codebase-model`, `scan-git`, and similar authoring workflows. `operations/optimization/guides/` often serves as the precursor surface for these skills: a weakly formalized procedural guide becomes a strongly formalized, parameterizable skill once the workflow is stable enough. A distribution that implements kernel operations as model calls is non-conforming with the deterministic-operation contract — and is wasting budget.
