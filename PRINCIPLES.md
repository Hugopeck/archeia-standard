...
Latent work belongs in skills layered above the kernel: `review-product`, `clarify-idea`, `write-codebase-model`, `scan-git`, and similar authoring workflows. `operations/guides/` often serves as the precursor surface for these skills: a weakly formalized procedural guide becomes a strongly formalized, parameterizable skill once the workflow is stable enough. A distribution that implements kernel operations as model calls is non-conforming with the deterministic-operation contract — and is wasting budget.
